...
void	draw_line(t_data *data, t_position starting_position, t_position end_position)
{
	...
	delta.y = (end_position.y - starting_position.y);
	delta.x = (end_position.x - starting_position.x);
	pixels = sqrt((delta.x * delta.x) + (delta.y * delta.y));
	delta.y /= pixels;
	delta.x /= pixels;
	pixel.x = starting_position.x;
	pixel.y = starting_position.y;
	while (pixels)
	{
...
t_position	find_ray_colision(t_position starting_position, float angle, char** map)
{
	int		a;
	int		x;
	int		y;
